...
// File: contracts/FantiumMetaToken.sol
...
contract FantiumMetaToken is ERC1155, Ownable, ERC1155Supply {
    ...
    event SomethingRead(string something);
    event SomeIntRead(uint something);

    uint256 public constant TOKENS_PER_COLLECTION = 1000;
    ...
    function mint(uint256 collectionId, bytes memory data) external {
    }
        ...
        override(ERC1155, ERC1155Supply)
    {
        if (from == address(0)) { // We're minting
            ...
            for(uint i = 0; i < ids.length; i++) {
                ...
                uint256 collectionId = ids[i] / (TOKENS_PER_COLLECTION);

                require(collections[collectionId].supply == ids[i], "Wrong token id sequence");
                require(isAllowedToMint(collectionId, operator), "Address not allowed to mint new tokens");

                emit SomethingRead("BEFORE");

                string memory tokenInfo = strings.toString(strings.split(s, tokenDelimiter));

                string memory tokenUri = _newTokenDetails(tokenInfo, ids[i], collectionId);

                emit SomethingRead(tokenInfo);

                // strings.slice memory inTokenDelimiter = strings.toSlice("-");


                // strings.slice memory left = strings.toSlice(tokenInfo);

                // string memory tokenURI = strings.toString(strings.split(left, inTokenDelimiter));

                // emit SomethingRead(tokenURI);

                // emit SomethingRead("AND NOW");


                // string memory another = strings.toString(strings.split(left, inTokenDelimiter));

                // uint parsed = strings.stringToUint(another);

                // emit SomeIntRead(parsed);
    ...
    function _newTokenDetails(string memory tokenData, uint256 tokenId, uint256 collectionId) private returns (string memory) {
        strings.slice memory inTokenDelimiter = strings.toSlice("-");
        ...
        uint tokenShare = strings.stringToUint(another);

        emit SomeIntRead(tokenShare);

        require(tokenShare <= collections[collectionId].shareLeft);
    ...
    function tokenDetails(uint256 tokenId) external view returns (uint256, string memory) {
